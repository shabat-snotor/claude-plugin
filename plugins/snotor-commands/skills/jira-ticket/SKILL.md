...
**Never cite a rule, document, or checklist by number.** State what the rule requires, or just state the consequence and move on. Write "so `apps/payments` may import it later", never "so `apps/payments` may import it under boundary rule 4". Never "per ADR-001", "item 12 of the checklist", "rule 3 of the module boundaries", "see constraint 2 above". The number means nothing to the reader without opening another document, and it is wrong the moment that document is reordered or renumbered. If the rule's content matters to the work, put it in a clause: "so nothing outside the adapter can import the SDK". If it does not matter, drop the reference entirely. This is the same rule as linking every document reference, applied to the numbered items inside one: a number is never a reference.
